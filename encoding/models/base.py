...
class BaseNet(nn.Module):
    ...
    def base_forward(self, x):
       ...
       # 获取第一层模块
        conv1_module = self.pretrained.conv1
        
        # 查找实际的卷积层
        if isinstance(conv1_module, nn.Sequential):
            # 在Sequential中查找第一个Conv2d层
            conv_layer = None
            for module in conv1_module:
                if isinstance(module, nn.Conv2d):
                    conv_layer = module
                    break
            if conv_layer is None:
                raise RuntimeError("No Conv2d layer found in self.pretrained.conv1 Sequential")
        else:
            conv_layer = conv1_module

        # 动态调整第一层卷积
        if conv_layer.in_channels != x.shape[1]:
            new_conv = nn.Conv2d(
            ...
            with torch.no_grad():
                if conv_layer.weight.shape[1] == 3:
                    ...
                    new_conv.weight.copy_(mean_weight.repeat(1, x.shape[1]//3, 1, 1))
                else:
            ...
            if isinstance(conv1_module, nn.Sequential):
                ...
                for module in conv1_module:
                    if module is conv_layer:
                        new_seq.append(new_conv)
                        replaced = True
                    else:
                        new_seq.append(module)
                if not replaced:
                    raise RuntimeError("Failed to replace Conv2d layer in Sequential")
                self.pretrained.conv1 = nn.Sequential(*new_seq)
            else:
                self.pretrained.conv1 = new_conv
                
            # 确保新层在正确的设备上
            self.pretrained.conv1 = self.pretrained.conv1.to(x.device)
